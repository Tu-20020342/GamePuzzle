...
class Board {
  ...
  reset() {
    this.score = 0;
    this,this.grid = this.generateWhiteBoard();
    this.gameOver = false;
    this.drawBoard();
  }
  ...
  drawCell(xAxis, yAxis, colorId) {
    this.ctx.fillStyle =
      COLOR_MAPING[colorId] || COLOR_MAPING[WHITE_COLOR_ID];
    this.ctx.fillRect(
      xAxis * BLOCK_SIZE,
      yAxis * BLOCK_SIZE,
  ...
  handleCompeleteRows() {
    ...
    const newRow = Array.from({length: newScore}, () => Array(COLS).fill(WHITE_COLOR_ID))
...
function generateNewBrick() {
  brick = new Brick(Math.floor(Math.random() * 10) % BRICK_LAYOUT.length); //tao ra 1 id bat ki tu 0 -> 6 ung voi tung vien gach
}
